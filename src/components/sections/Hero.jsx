    ...
    <section style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", overflow: "hidden" }}>
      ...
      <motion.div style={{ y, opacity, width: "100%", paddingTop: 90 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 clamp(1.5rem, 5vw, 6rem)" }}>
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#10b981", boxShadow: "0 0 12px #10b981" }} />
            <span className="mono" style={{ fontSize: "0.68rem", color: "var(--text-dim)", letterSpacing: "0.22em" }}>AVAILABLE FOR OPPORTUNITIES</span>
          </motion.div>

          <div style={{ overflow: "hidden" }}>
            <motion.h1 initial={{ opacity: 0, y: 80 }} animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 1, delay: 0.6, ease: [0.22, 1, 0.36, 1] }}
              style={{ fontSize: "clamp(3.4rem, 9.5vw, 8rem)", fontWeight: 800, lineHeight: 0.93, letterSpacing: "-0.035em", marginBottom: "0.15rem", color: "var(--text)" }}>
              Harshad
            </motion.h1>
          </div>
          <div style={{ overflow: "hidden" }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ width: 2, height: 38, background: "linear-gradient(to bottom, var(--primary), var(--secondary))", borderRadius: 999, flexShrink: 0 }} />
            <AnimatePresence mode="wait">
              <motion.p key={wordIdx}
                initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -14 }}
                transition={{ duration: 0.38 }}
                style={{ fontSize: "clamp(1.1rem, 2.5vw, 1.5rem)", fontWeight: 500, color: "var(--text-dim)" }}>
                {words[wordIdx]}
              </motion.p>
            </AnimatePresence>
          </motion.div>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.9, delay: 1.2 }}
            style={{ maxWidth: 520, fontSize: "1.05rem", lineHeight: 1.8, color: "var(--text-dim)", marginBottom: "3rem" }}>
            B.Tech IT · Walchand Institute of Technology · {" "}
            <strong style={{ color: "var(--primary)", fontWeight: 700 }}>CGPA 9.17</strong> · Building scalable full-stack applications and clean backend architectures using the MERN stack, Java, and DSA.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
            ...
            style={{ display: "flex", gap: "clamp(1.5rem, 5vw, 4rem)", marginTop: "5.5rem", flexWrap: "wrap" }}>
            {[["9.17", "CGPA"], ["1+", "Internship"], ["2+", "Certifications"], ["15+", "REST APIs Built"]].map(([val, lbl]) => (
              <div key={lbl}>
                <div style={{ fontSize: "clamp(1.8rem, 3.5vw, 2.4rem)", fontWeight: 800, color: "var(--text)", letterSpacing: "-0.04em" }}>{val}</div>
                <div className="mono" style={{ fontSize: "0.58rem", color: "var(--text-dim)", letterSpacing: "0.22em", textTransform: "uppercase", marginTop: 2 }}>{lbl}</div>
              </div>
            ))}
          </motion.div>
        </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }}
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 1.7 }}
          style={{ width: 22, height: 35, border: "1px solid var(--border)", borderRadius: 99, display: "flex", justifyContent: "center", paddingTop: 7 }}>
          <div style={{ width: 2, height: 7, borderRadius: 99, background: "var(--primary)" }} />
        </motion.div>
      </motion.div>
    </section>
  );
