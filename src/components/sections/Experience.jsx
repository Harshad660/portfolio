...
export const Experience = () => (
  <section id="experience" style={{ position: "relative", padding: "8rem 0", overflow: "hidden" }}>
    <Glow x="78%" y="40%" size={450} color="var(--primary-glow)" />
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 clamp(1.5rem, 5vw, 6rem)" }}>
      <FadeUp><SectionLabel num={5} label="Experience" /></FadeUp>
      <FadeUp delay={0.1}>
        <h2 style={{ fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 800, lineHeight: 1.15, marginBottom: "4rem", letterSpacing: "-0.025em", color: "var(--text)" }}>
          Where I've applied<br /><span className="shimmer-text">my skills.</span>
        </h2>
      </FadeUp>
      <div style={{ position: "relative" }}>
        <div style={{ position: "absolute", left: 19, top: 8, bottom: 8, width: 1, background: "var(--border)" }} />
          ...
          {EXPERIENCES.map((exp, i) => (
            <FadeUp key={i} delay={i * 0.1}>
              <div style={{ display: "flex", gap: "2rem" }}>
                ...
                <motion.div
                  whileHover={{ borderColor: exp.color, boxShadow: "0 8px 40px var(--primary-glow)" }}
                  style={{
                    flex: 1, padding: "1.5rem 1.75rem", minWidth: 0,
                    background: "var(--card-bg)", border: "1px solid var(--border)",
                    borderRadius: 18, transition: "all 0.35s",
                  }}>
                  <div style={{ marginBottom: "0.9rem" }}>
                    <h3 style={{ fontSize: "1.1rem", fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>{exp.role}</h3>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.55rem", flexWrap: "wrap" }}>
                      <span className="mono" style={{ fontSize: "0.65rem", color: exp.color, letterSpacing: "0.06em", fontWeight: 600 }}>{exp.org}</span>
                      ...
                      <span className="mono" style={{ fontSize: "0.6rem", color: "var(--text-dim)" }}>{exp.period}</span>
                    </div>
                  </div>
                  <ul style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                    {exp.points.map((pt, j) => (
                      <li key={j} style={{ display: "flex", gap: "0.65rem", alignItems: "flex-start", fontSize: "0.9rem", color: "var(--text-dim)", lineHeight: 1.7 }}>
